...
def rider_homepage():
    ...
    if request.method == 'POST':
        # Get form data, if not provided default to "Any"
        ...
        cursor = mysql.connection.cursor()
        cursor.execute('''INSERT INTO riderpreferences
    ...
    # Fetch existing preferences for the rider if available
    cursor.execute("SELECT * FROM riderpreferences WHERE riderId = %s", (riderID,))
    preferences = cursor.fetchone()
    cursor.close()

    # If no preferences are found, show the modal to input preferences
    if preferences is None:
        show_modal = True

    return render_template("RiderHomePage.html", show_modal=show_modal, preferences=preferences)
...
def rider_createtrip():
    ...
    if request.method == "POST":
        try:
            ...
            guest_count = no_of_passengers - 1  # Guests are total passengers minus 1 (the initiator)
            ...
            # Check if preferences differ from current ones
            if preferences:
            ...
            cursor.execute('''
                INSERT INTO trip (TripInitiatorID, TripInitiatorType, DriverID, Date, PickUpTime, DropOffTime, `From`, `To`, NoOfPassengers, GuestCount, Fare)
                VALUES (%s, %s, %s, %s, %s, NULL, %s, %s, %s, %s, %s)
            ''', (trip_initiator_id, 'Rider', driver_id, date, pick_up_time, from_location, to_location, no_of_passengers, guest_count, 0.0))
            trip_id = cursor.lastrowid  # Get the ID of the newly created trip
            mysql.connection.commit()

            # Insert TripRiders only for registered users (not guests)
...
def rider_history():
    ...
    try:
        ...
        ORDER BY trip.Date DESC, trip.PickUpTime DESC
        '''
        cursor.execute(query_initiated, (userID,))
        ...
        if initiated_trips:
            ...
            ORDER BY trip.Date DESC, trip.PickUpTime DESC
            '''
            cursor.execute(query_joined, (rider_id, tuple([trip['TripID'] for trip in initiated_trips])))
        else:
            query_joined = '''
            SELECT trip.TripID, trip.From, trip.To, trip.PickUpTime, trip.DropOffTime, trip.Date, 
                   trip.NoOfPassengers, trip.GuestCount, trip.Fare, 'Joined' AS role,
            ...
            ORDER BY trip.Date DESC, trip.PickUpTime DESC
            '''
            cursor.execute(query_joined, (rider_id,))
...
def ridersend_message():
    ...
    data = request.json
    rider_id = session['id']  # User's session ID (UserID)
    ...
    try:
        ...
        # Get the DriverID from the trip and find the associated UserID of the driver
        cursor.execute("""
        ...
        receiver_id = receiver['ReceiverID']  # Use the driver's UserID as ReceiverID
        ...
        # Fetch the sender's name and ID from the session
        sender_name = session.get('fullname', 'Unknown')
        sender_id = session.get('id')  # Ensure the SenderID is included

        cursor.close()

        # Return the response including the SenderID
        return jsonify({
            "success": True, 
            "sender_name": sender_name, 
            "sender_id": sender_id,  # Include SenderID
            "message": message
        }), 200
...
def driver_homepage():
    ...
    return render_template('DriverHomePage.html', show_modal=show_modal, preferences=preferences)
...
def driversend_message():
    ...
    driver_name = session.get('fullname', 'Unknown')  # Use the name from session
    trip_id = data.get('trip_id')
    message = data.get('message')
    ...
    try:
        ...
        # Get Rider's UserID from Trip table using TripInitiatorID
        query = """
        ...
        # Insert the message with the driver's name into the database
        insert_query = """
            INSERT INTO messages (TripID, SenderID, ReceiverID, Message, Timestamp)
            VALUES (%s, %s, %s, %s, NOW())
        """
...
def get_driver_messages(trip_id):
    try:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        query = """
            SELECT m.Message, m.SenderID, u.FullName AS SenderName, m.Timestamp
            FROM messages m
            JOIN user u ON m.SenderID = u.UserID
            WHERE m.TripID = %s
            ORDER BY m.Timestamp ASC
...
def driver_profile():
    user_id = session.get("id")  # Get UserID from session
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    ...
    return render_template("DriverProfile.html", user=user)
...
def driver_createtrip():
    ...
    if request.method == "POST":
        try:
            ...
            from_location = request.form['from']
            to_location = request.form['to']
            No_Passengers = request.form['tripPassengers']
            ...
            # Insert the new trip into the database
            cursor.execute('''
                INSERT INTO trip (TripInitiatorID, TripInitiatorType, DriverID, Date, PickUpTime,`From`, `To`, NoOfPassengers, Fare)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
            ''', (trip_initiator_id, 'Driver',driverID, date, pick_up_time, from_location, to_location, No_Passengers, 0.0))
            mysql.connection.commit()
